...
coordinates (r, c), where 0 <= r < R and 0 <= c < C. Additionally, we are given 
a cell in that matrix with coordinates (r0, c0).
...
1. For r in range(R):
    For c in range(C):
        Calculate manhattan distance from r0, c0 as-
        manhattan_dist = abs(r0 - r) + abs(c0 - c)
        Store [r,c] in sorted order of manhattan_dist
2. Store [r,c] in sorted order of manhattan_dist
3. Return result as list of [r,c] in sorted manhattan_dist
...
class Solution(object):
    def allCellsDistOrder(self, R, C, r0, c0):
        ...
        for r in range(R):
            for c in range(C):
                manhattan_dist = abs(r0 - r) + abs(c0 - c)
                store[manhattan_dist].append([r,c])
        result = []
        for entry in store:
            result.extend(entry)
        return result
