...
'''
ALGORITHM:
1. Time to inform employee = time_manager_gets_information + manager_wait_time
2. Insert all employees into a dictionary, in following format:
    <manager-id>:<list of reporting employee ids>
2. Keep a store array of size n(num total employees), where store[i] is the time
   i-th employee gets the information
3. Starting from the headID, start updating the inform time for each employee 
   using the formula in (1). 
   Note:- Use BFS traversal 
4. Return max(store) 
...
class Solution(object):
    ...
    def numOfMinutes(self, n, headID, manager, informTime):
        ...
        emp_tree = defaultdict(list)
        for i in range(len(manager)):
            manager_id = manager[i]
            emp_tree[manager_id].append(i)
        ...
        while cnt < n:
            ...
            emps = emp_tree[emp]
            for e in emps:
                store[e] = store[emp] + informTime[emp]
            q.extend(emps)
        return max(store)