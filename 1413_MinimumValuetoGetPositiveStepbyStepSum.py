...
startValue. In each iteration, you calculate the step by step sum of startValue 
...
'''
ALGORITHM:
1. Find result of each step sum and keep track of minimum sum found so far 
2. the startValue to keep the step sum always positive will be abs(min_sm) + 1
...
class Solution(object):
    def minStartValue(self, nums):
        ...
        for n in nums:
            sm += n
            min_step_sm = min(min_step_sm, sm)
        
        
        return abs(min_step_sm) + 1
