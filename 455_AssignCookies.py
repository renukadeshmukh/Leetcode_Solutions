...
are 1, 2. You have 3 cookies and their sizes are big enough to gratify all of the 
...
ALGORITHM:
1. Sort G and S
2. While i < len(g) and j < len(s)
   if cookie of s[j] satisfies child i, increment i and j
   else increment j and check if next cookie will satisfy the child.  

RUNTIME COMPLEXITY: O(NlogN + MlogM) ; where N = len(g) and M = len(s)
