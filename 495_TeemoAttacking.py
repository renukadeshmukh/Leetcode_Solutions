...
'''
ALGORITHM:
1. For each element in the array, if timeSeries[i] + duration > timeSeries[i+1],
   this means there will be an overlap, so only consider 
   timeSeries[i+1] - timeSeries[i] seconds

   if timeSeries[i] + duration > timeSeries[i+1]:
        total_time += (timeSeries[i+1] - timeSeries[i])
2. Else consider the entire 'duration' seconds
