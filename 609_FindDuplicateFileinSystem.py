...
need to find out all the groups of duplicate files in the file system in terms of their paths. A group of 
duplicate files consists of at least two files that have exactly the same content.
    ...
    def findDuplicate(self, paths):
        ...
        for path in paths:
            ...
            for i in xrange(1, len(files)):
                content = files[i][files[i].index('('):] 
                if content in content_map:
                    content_map[content].append('/'.join([files[0], files[i][ : files[i].index('(')]]))
                else:
                    content_map[content]= ['/'.join([files[0], files[i][ : files[i].index('(')]])]
       
        return [a for a in content_map.values() if len(a) > 1]
