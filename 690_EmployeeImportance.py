...
employee 1 has a data structure like [1, 15, [2]], and employee 2 has 
...
'''
ALGORITHM:
1. Save all employees in a hashmap with key = employee_id with easy access.
2. Use a queue to track employee and all the subordinates. 
3. While len(queue) > 0, pop element from 0th index.
4. Add the importance of this employee to total. 
5. Enqueue all the subordinates for this employee
...
class Solution(object):
    def getImportance(self, employees, id):
        ...
        while len(queue) > 0:
            emp_id = queue.pop(0)
            total_importance += emp_map[emp_id].importance
            queue.extend(emp_map[emp_id].subordinates)
