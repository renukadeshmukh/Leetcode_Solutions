...
see the examples below for details. After removing the comments from the source 
code, return the source code in the same format.
...
class Solution(object):
    def removeComments(self, source):
        ...
        """
        result = []
        block_comm_start, block_comm_end = False, False
        for line in source:
            index_line_comm = line.find('//')
            index_block_comm = line.find('/*')
            if index_line_comm != -1:
                line = self.remove_line_comment(line, index_line_comm)
                if line:
                    result.append(line)
        return result
    ...
    def remove_line_comment(self, line, index):
