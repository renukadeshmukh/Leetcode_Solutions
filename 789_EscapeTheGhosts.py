...
destination is (target[0], target[1]). There are several ghosts on the map, the 
i-th ghost starts at (ghosts[i][0], ghosts[i][1]).
...
'''
ALGORITHM:
1. Calculate distance of pacman from target.
    d = abs(x1 - 0) + abs(y-0), where x,y is target and pacman moves one unit 
    vertically or horizontally at a time.
2. For each ghost, calcute the distance of ghost from target.
    d = abs(xi - x1) + abs(yi-y1), where xi and yi are initial co-ordinates of 
    ghost i. And the ghost has to reach x1, y1 to beat pacman
3. If any ghost has distance to target less than pacman, pacman will lose. In
   that care, return False. Else return True.
 
RUNTIME COMPLEXITY: O(N) where N is number of ghosts.
...
class Solution(object):
    def escapeGhosts(self, ghosts, target):
        ...
        :rtype: bool
        """
        pacman_dist = self.get_distance(target[0], target[1])
        #print pacman_dist
        
        for ghost in ghosts:
            ghost_dist =  self.get_distance(ghost[0]-target[0], ghost[1]-target[1])
