...
100 units, it is written on the next line. We are given an array widths, an array where widths[0] 
...
Algorithm:
1. Iterate on S. 
2. If current length of line + units of current char > 100, increment line count and update units in new line
    Else add the units to current line
...
class Solution(object):
    def numberOfLines(self, widths, S):
        ...
        lines, units = 1,0
        for c in S:
            c_units = widths[ord(c) - 97]
            if units + c_units > 100:
                lines += 1
                units = c_units
            else:
                units += c_units
