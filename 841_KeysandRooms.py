...
ALGORITHM:
DFS
RUNTIME COMPLEXITY: O(N+E), where N is number of rooms, and E is number of keys.
...
class Solution(object):
    def canVisitAllRooms(self, rooms):
        ...
        unvisited = deque()
        self.addUnvisitedRooms(0, visited, unvisited, rooms)
        while len(unvisited) > 0:
            curKey = unvisited.pop()
            self.addUnvisitedRooms(curKey, visited, unvisited, rooms)

        return all(visited)   

    def addUnvisitedRooms(self, curKey, visited, unvisited, rooms ):
        visited[curKey] = True
