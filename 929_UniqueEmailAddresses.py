...
Besides lowercase letters, these emails may contain '.'s or '+'s. If you add 
periods ('.') between some characters in the local name part of an email address, 
mail sent there will be forwarded to the same address without dots in the local 
name.  For example, "alice.z@leetcode.com" and "alicez@leetcode.com" forward to 
...
'''
ALGORITHM:
1. Split email into local and domain
2. Split local at '+' and take first part
3. Replace '.' with '' (empty string)
4. Reform email by joining local and domain 
5. Add new email to set
6. Return size of set.
 
RUNTIME COMPLEXITY: O(N + M) for N string of length M
SPACE COMPLEXITY: O(1) (O(N) to save result)
'''
